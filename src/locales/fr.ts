...
export default {
  ...
  defaults: {
    ...
    dsCalendarApp: {
      ...
      labels: {
        next: (type: VCalendarType) => type ? 'Suivant ' + (type.label as string).toLowerCase() : 'Suivant',
        prev: (type: VCalendarType) => type ? 'Précédent ' + (type.label as string).toLowerCase() : 'Précédent',
